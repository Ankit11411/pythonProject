...
def clock(counter, curr_time, count):
    global string
    tt = datetime.datetime.fromtimestamp(counter)
    string = tt.strftime("%H:%M:%S")
    # print(string)
    curr_time.config(text=string)
    curr_time.after(1000, lambda: [clock(counter, curr_time, count)])
    counter += 1
...
def cust_in(user_get_in):
    ...
    for i in range(no_of_pc):
        # print(a)
        # print(cust_queue.values())
        # print(cust_queue)
        for j in range(no_of_pc):
            print(cust_queue[j + 1])
            print(user_get_in, "AAAAAAA")
            if cust_queue[j + 1] == user_get_in:
                messagebox.showinfo("Exist!", "Customer already allotted seat!")
                return

        if cust_queue[i + 1] == 'empty':
            l = Label(master, text=user_get_in)
            l.place(x=50, y=a)
            curr_time = Label(master, text="")
            curr_time.place(x=150, y=a)
            counter = 66600
            clock(counter, curr_time, count)
            # clist.destroy()
            cust_queue[count] = user_get_in
            break
        count = count + 1
        # b = Button(master, text='REMOVE', command=lambda: [remo(l, b, i)])
        # b.place(x=800, y=a)
        a = a + 50
        print("ASD", a)
...
def windows():
    global master, id, cust_queue, counter, curr_time
    curr_time = []

    cust_queue = {1: 'empty', 2: 'empty', 3: 'empty', 4: 'empty', 5: 'empty', 6: 'empty', 7: 'empty', 8: 'empty'}
    id = 0
    master = Tk()
